...
OMG_API void turnto(SoursePlayer*p, OtherPlayer *pos)
{
...
OMG_API bool ableToAtt(SoursePlayer*p, int pur, OtherPlayer*op1, OtherPlayer*op2, OtherPlayer*op3)
{
	...
	else if (aabs(x1 - x2) <= eps && aabs(y1 - y2) <= 230 && aim->isjump())
	{
		turnto(p, aim);
		return true;
	}
...
OMG_API bool ableS(SoursePlayer*p, int pur, int kind, OtherPlayer*op1, OtherPlayer*op2, OtherPlayer*op3)
{
	...
	for (int po = 1; po >= 3; po++)
	{
		...
		if (aabs(x1 - x2) <= eps && aabs(y1 - y2) <= 50)
		{
			turnto(p, aim);
			return true;
		}
...
OMG_API int locked(int pur, OtherPlayer*op1, OtherPlayer*op2, OtherPlayer*op3)
{
	OtherPlayer *aim = op1;
	int pos = -1;
	double hp = 9999; // < 50
...
OMG_API bool playrole(SoursePlayer*p, OtherPlayer*op1, OtherPlayer*op2, OtherPlayer*op3, int*box) {
	...
	//Low HP
	if (p->gethp() < 42 && p->getmp() < basmp + 10)
		runTo(p, pbox[0].x, pbox[0].y );
